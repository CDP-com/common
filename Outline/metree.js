//Total Menus in This File: 1
//Locations:
//#10: http://snapback-apps.com/analyzedisk/
//#2: http://snapback-apps.com/helloworld/
//#12: http://snapback-apps.com/oominstaller/
//#4: http://snapback-apps.com/setreboot/
//#6: http://snapback-apps.com/skypehistory/
//#11: http://snapback-apps.com/skype2/
//#8: http://snapback-apps.com/welcomeoca/
//#9: http://snapback-apps.com/developers/
 
//Menu Number: 1
//Menu Name: Static App Menu
 
var menu01 =
{
   "nodes" :
   [
   {
      "ilvl" : 0,
      "name" : "Navigation Outline",
      "url" : "#",
      "title" : "The Navigation Outline is a categorized list of apps and pages. This list can be traversed by using the arrow keys. You can make a selection by using the enter key.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 1,
      "name" : "Home - SnapBack Apps Platform",
      "url" : "../welcomeoca/main.html",
      "title" : "App that Introduces and Describes the Platform. ",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 1,
      "name" : "Platform Installation",
      "url" : "../oominstaller/main.html",
      "title" : "App that Installs the Platform on a Windows PC",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 1,
      "name" : "Featured Apps",
      "url" : "#",
      "title" : "These are new apps. The development of these apps is very active and your input is valued.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "Manage My PCs",
      "url" : "../managemypcs/main.html",
      "title" : "Use the Manage MY PCs app to run common apps on all of your PCs at once.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "Analyze Disk",
      "url" : "../analyzedisk/main.html",
      "title" : "This app Checks for Hard Disk Errors and displays them.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "Max Script Statements",
      "url" : "../maxscriptstatements/main.html",
      "title" : "This app allows you to change the value of the Windows registry setting that triggers browser prompts that inform the user of a script that is taking a long time to execute. It also allows or encourages the user to stop the execution of the script. This app sets the value to a value that will allow the scripts to run without the message. You can also change it back to the default value.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "Reboot After Update",
      "url" : "../setreboot/main.html",
      "title" : "Turn off Reboot after a windows Update",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "Multiple Skype Sessions",
      "url" : "../skype2/main.html",
      "title" : "This app allows you to Run Multiple Skype Instances",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "Skype History",
      "url" : "../skypehistory/main.html",
      "title" : "This app will Create a Skype history folder on your desktop",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "Show Snapshot VSS",
      "url" : "../snapshotsvss/main.html",
      "title" : "This app will create a Microsoft Windows Shadow Copy of a volume",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 1,
      "name" : "Create an App",
      "url" : "../developers/main.html",
      "title" : "App that Installs the Developers Environment and contains guides for developing apps.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "App Template",
      "url" : "../apptemplate/main.html",
      "title" : "App Template is a template for developers to use as a sample to help create their own apps.  It is used to help create the look and feel for all our apps.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "Hello World  (example app)",
      "url" : "../helloworld/main.html",
      "title" : "Hello world is an example app. It is used to help establish the look and feel for all our apps.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 2,
      "name" : "Style (example app)",
      "url" : "../styleapp/main.html",
      "title" : "Style is an example app. It is used to help establish the look and feel for all our apps.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 1,
      "name" : "Notices",
      "url" : "../notices/main.html",
      "title" : "Notices is an app that displays alerts and notifications to the user of the SnapBack Platform.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 1,
      "name" : "Settings",
      "url" : "../settings/main.html",
      "title" : "Settings is an app that configures the SnapBack Platform for the user.",
      "comment" : "",
      "target" : "_self",
      "expanded" : true
   }
   ,
   {
      "ilvl" : 0,
      "name" : "End",
      "url" : "",
      "title" : "End",
      "comment" : "Last Entry - HIDE ME LATER",
      "target" : "",
      "expanded" : false
   }
   ]
}
;
 

